...
class KmlFeatureParser {
  ...
  private static final String GEOMETRY_REGEX =
      "Point|LineString|Polygon|MultiGeometry|Track|MultiTrack";
  ...
  private static final String PROPERTY_REGEX =
      "name|description|drawOrder|visibility|open|address|phoneNumber";
  ...
  /**
   * Internal helper class to store latLng and altitude in a single object. This allows to parse
   * [lon,lat,altitude] tuples in KML files more efficiently. Note that altitudes are generally
   * optional so they can be null.
   */
  ...
  /**
   * Creates a new Placemark object (created if a Placemark start tag is read by the XmlPullParser
   * and if a Geometry tag is contained within the Placemark tag) and assigns specific elements read
   * from the parser to the Placemark.
   */
  ...
  static KmlGroundOverlay createGroundOverlay(XmlPullParser parser)
    ...
    latLonBox =
        createLatLngBounds(
            compassPoints.get("north"),
            compassPoints.get("south"),
            compassPoints.get("east"),
            compassPoints.get("west"));
  ...
  /**
   * Retrieves a url from the "href" tag nested within an "Icon" tag, read by the XmlPullParser.
   *
   * @return An image url
   */
  ...
  /**
   * Creates a new Geometry object (Created if "Point", "LineString", "Polygon" or "MultiGeometry"
   * tag is detected by the XmlPullParser)
   *
  ...
  /**
   * Creates a new KmlPolygon object. Parses only one outer boundary and no or many inner boundaries
   ...
   */
  private static LatLngBounds createLatLngBounds(
