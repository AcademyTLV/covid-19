...
data class InfectedLocationsMOHResponse(
    ...
    fun toInfectedLocations(): List<InfectedLocationModel> {
        return features.map { mohModel ->
            InfectedLocationModel(
                id = mohModel.id,
                startTime = Date(mohModel.properties.fromTime),
                endTime = Date(mohModel.properties.toTime),
                lat = mohModel.geometry.coordinates[1],
                lon = mohModel.geometry.coordinates[0],
                radius = 30.0,
                name = mohModel.properties.place,
                comments = mohModel.properties.comments
            )
        }
    }
}
